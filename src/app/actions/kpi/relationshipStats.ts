 ...
 * @constructor
 */
export default async function RelationshipStats(req: any, res: any, next: any) {
    const kpiClient = new KPIClient();
    const jwt = JWT.parseFromHeader(
        req.header('Authorization')
    );

    return kpiClient.getRelationshipStats(
 ...
 * @ignore
 * @swagger
 * /v1/kpi/relationship-stats:
 *   get:
 *     security:
 *       - Bearer: []
 *     description: Get count of different relationship status types
 *     tags: ['kpi']
 *     produces:
 *       - application/json
 *     responses:
